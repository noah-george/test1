...
function initWebSocket() {
    ...
    websocket.onmessage = event => {
        ...
        console.log(transcript_data.language_probability);
    
        if (transcript_data.language_probability > 0.9) {
            console.log("spoken text:", transcript_data.text);
        } else {
            console.log("Speech not recognized. Could be just noise or hallucinations:", transcript_data.text);
        }
        
    
        // Update the transcription with timestamp
        transcript_data.timestamp = timestamp;
...
function updateTranscription(transcript_data) {
    ...
    const languageDiv = document.getElementById('detected_language');
    const timestamp = new Date().toLocaleTimeString();
    const asrType = transcript_data.asr_type;
    console.log(asrType)
    ...
    paragraph.textContent = `[${timestamp}] `;
        ...
        if (transcript_data['words'] && transcript_data['words'].length > 0) {
            ...
            // Add the paragraph to the transcriptionDiv
            transcriptionDiv.appendChild(paragraph);
        } else {
            // Fallback to plain text
            const paragraph = document.createElement('p');
            paragraph.textContent += transcript_data['text'];
            transcriptionDiv.appendChild(paragraph);
        }
    // } 
    //else {
    ...
    // // Update the language information
    if (transcript_data['language'] && transcript_data['language_probability']) {
        languageDiv.textContent = `[${timestamp}] ${transcript_data['language']} (${transcript_data['language_probability'].toFixed(2)})`;
     }

    // // Update the processing time, if available
    const processingTimeDiv = document.getElementById('processing_time');
    if (transcript_data['processing_time']) {
        processingTimeDiv.textContent = `[${timestamp}] Processing time: ${transcript_data['processing_time'].toFixed(2)} seconds`;
    }
    }
